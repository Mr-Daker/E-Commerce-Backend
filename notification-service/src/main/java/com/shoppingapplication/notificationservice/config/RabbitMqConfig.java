package com.shoppingapplication.notificationservice.config;
...
public class RabbitMqConfig {
    @Bean
    TopicExchange orderEventsExchange(@Value("${order.events.exchange}") String name) {
        return new TopicExchange(name, true, false);
    }

    @Bean
    Queue orderEventsQueue(@Value("${order.events.queue}") String name,
                           @Value("${order.events.dead-letter-exchange}") String deadLetterExchange) {
        return QueueBuilder.durable(name).deadLetterExchange(deadLetterExchange).build();
    }
